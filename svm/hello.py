...
# Get the values of n, d, and epsilon
...
# Transform original points using JL Lemma
k = int(input("Enter the number of dimensions after transformation (k): "))
transformation_matrix = np.random.normal(size=(d, k)) / np.sqrt(k)
transformed_points = np.dot(original_points, transformation_matrix)
